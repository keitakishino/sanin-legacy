  ...
  describe "GET /" do
    ...
    context "when user is logged in" do
      ...
      context "when user has incomplete trades" do
        ...
        it "displays pending trade with status badge" do
          get "/"
          expect(response.body).to include(event.title)
          expect(response.body).to include("未処理")
        end

        it "displays in_progress trade with status badge" do
          get "/"
          expect(response.body).to include("進行中")
        end

        it "displays trade aggregates" do
          get "/"
        ...
        it "displays incomplete trades in order (newest first)" do
          event3 = create(:event)
          event4 = create(:event)
        ...
        it "displays maximum 3 incomplete trades when more than 3 exist" do
        ...
        it "does not display completed trades" do
          ...
          expect(response.body).not_to include(completed_trade.created_at.to_s.split[0])
        end

        it "does not display cancelled trades" do
          ...
          expect(response.body).not_to include(cancelled_trade.created_at.to_s.split[0])
        end
      end
      ...
      context "when other user has incomplete trades" do
        ...
        it "does not display other user's trades" do
          ...
          # Should not display any trade-related content
          expect(response.body).to include("トレードはまだありません")
        end
      end
